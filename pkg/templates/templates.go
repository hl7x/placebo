...
{{.MRN}},{{.VisitId}},{{.FirstName}},{{.LastName}},{{.DOB}},M,{{.PatientAddress.StructureNumber}} {{.PatientAddress.Street}},{{.PatientAddress.RegionInfo.City}},{{.PatientAddress.RegionInfo.State}},10119,USA,{{.ArrivalDate}} 06:00 PM,17:30 EDT,DEP1,{{.Phone}},English,JACK RYAN,{{.Appointment}} 03:00 PM,Scheduled,TESTALTID283860873785909398765{{ end  }}
`)
...
func SimpleHl7Info() []byte {
	...
	hl7Example := []byte(`MSH|^~\&|SENDAPP|PLACEBO|RECVAPP|LAB|{{.EventDate.HL7}}0800||ADT^A01|12345|P|2.3|
EVN|A01|{{.EventDate.HL7}}0800||
PID|1|56789|{{.MRN}}||{{.LastName}}^{{.FirstName}}||{{.DOB.HL7}}|M|||{{.PatientAddress.StructureNumber}} {{.PatientAddress.Street}}^^{{.PatientAddress.RegionInfo.City}}^{{.PatientAddress.RegionInfo.State}}^12346||{{.Phone}}|
PV1|1|IP|PUNIT^RM 123^P^PUNIT|ER|||15551234567^HOUSE^GREG|1255568135^BETA^ALPHA|30104384^SMITH^JOHN|HIM||||Phys/Clinic|||123456789^BETA^ALPHA|TESTX|{{.VisitId}}|||||||||||||||||||||||||{{.ArrivalDate.HL7}}|||
`)
...
func DischargeHl7Info() []byte {
	...
	dischargeHl7 := []byte(`
MSH|^~\&|SENDAPP|PLACEBO|RECVAPP|LAB|{{.EventDate.HL7}}0800||ADT^A03|12345|P|2.3|
EVN|A03|{{.EventDate.HL7}}0800||
PID|1|56789|{{.MRN}}||{{.LastName}}^{{.FirstName}}||{{.DOB.HL7}}|M|||{{.PatientAddress.StructureNumber}} {{.PatientAddress.Street}}^^{{.PatientAddress.RegionInfo.City}}^{{.PatientAddress.RegionInfo.State}}^12346||{{.Phone}}|
PV1|1|DC|PUNIT^RM 123^P^PUNIT|ER|||15551234567^HOUSE^GREG|1255568135^BETA^ALPHA|30104384^SMITH^JOHN|HIM||||Phys/Clinic|||123456789^BETA^ALPHA|TESTX|{{.VisitId}}||||||||||||||||||||||||||{{.DischargeDate.HL7}}||
...
func PreadmitHl7Info() []byte {
	...
	preadmit := []byte(`
MSH|^~\&|SENDAPP|PLACEBO|RECVAPP|LAB|{{.EventDate.HL7}}0800||ADT^A05|12345|P|2.3|
EVN|A05|{{.EventDate.HL7}}0800||
PID|1|56789|{{.MRN}}||{{.LastName}}^{{.FirstName}}||{{.DOB.HL7}}|M|||{{.PatientAddress.StructureNumber}} {{.PatientAddress.Street}}^^{{.PatientAddress.RegionInfo.City}}^{{.PatientAddress.RegionInfo.State}}^12346||{{.Phone}}|
PV1|1|PR|PUNIT^RM 123^P^PUNIT|ER|||15551234567^HOUSE^GREG|1255568135^BETA^ALPHA|30104384^SMITH^JOHN|HIM||||Phys/Clinic|||123456789^BETA^ALPHA|TESTX|{{.VisitId}}|||||||||||||||||||||||||{{.ArrivalDate.HL7}}|||
`)
...
func ScheduledPatientInfo() []byte {
	...
	scheduleSiu := []byte(`
MSH|^~\&|SENDAPP|PLACEBO|RECVAPP|LAB|{{.EventDate.HL7}}0800||SIU^S12|12345|P|2.3|
SCH|12345|12345|OFFICE^123^PUNIT|{{.Appointment.HL7}}|0900|||60|M^Minutes|Regular Appointment||ID123|||Scheduled|
PID|1|56789|{{.MRN}}||{{.LastName}}^{{.FirstName}}||{{.DOB.HL7}}|M|||{{.PatientAddress.StructureNumber}} {{.PatientAddress.Street}}^^{{.PatientAddress.RegionInfo.City}}^{{.PatientAddress.RegionInfo.State}}^12345||{{.Phone}}|
RGS|1|A|
`)
...
RF1|NEW|ASAP|NVgastro|||{{.VisitId}}|{{.Appointment.HL7}}|20250124|202401231139|Referral
AUT||||20240124|
