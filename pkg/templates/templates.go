...
PV1|1|IP|PUNIT^RM 123^P^PUNIT|ER|||15551234567^HOUSE^GREG|1255568135^BETA^ALPHA|30104384^SMITH^JOHN|HIM||||Phys/Clinic|||123456789^BETA^ALPHA|TESTX|{{.VisitId}}|||||||||||||||||||||||||{{.Hl7Info.HL7Arrival}}|||
`)
...
PID|1|56789|{{.MRN}}||{{.LastName}}^{{.FirstName}}||{{.Hl7Info.HL7DOB}}|M|||{{.PatientAddress.StructureNumber}} {{.PatientAddress.Street}}^^{{.PatientAddress.RegionInfo.City}}^{{.PatientAddress.RegionInfo.State}}^12346||{{.Phone}}|
PV1|1|PR|PUNIT^RM 123^P^PUNIT|ER|||15551234567^HOUSE^GREG|1255568135^BETA^ALPHA|30104384^SMITH^JOHN|HIM||||Phys/Clinic|||123456789^BETA^ALPHA|TESTX|{{.VisitId}}|||||||||||||||||||||||||{{.Hl7Info.HL7Arrival}}|||
`)
...
RF1|NEW|ASAP|NVgastro|||{{.VisitId}}|{{.Hl7Info.HL7Appointment}}|20250124|202401231139|Referral
AUT||||20240124|
